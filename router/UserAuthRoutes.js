...
UserAuthRoutes.post('/users/newadminuser', async (req, res) => {
	...
	try {
        ...
        const user = await adminUserModel.create(newUser)
		return res.status(201).send({ status: 'ok', user: user })
	} catch (err) {
		// res.json({ status: 'error', error: 'Duplicate email' })
		return res.status(500).send({"message":"POST operation failed while creating admin user"})
	}
})
...
UserAuthRoutes.post('/users/adminlogin', async (req, res) => {
...
UserAuthRoutes.get('/checkAdminLoggedIn', async (req, res) => {
	const token = req.headers['x-access-token']
	try {
		const decoded = jwt.verify(token, 'secret123')
		const email = decoded.email
